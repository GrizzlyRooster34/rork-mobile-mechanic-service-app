...
import { devMode, devCredentials, isDevCredentials, getDevUser } from './utils/dev';
import { getBreadcrumbs, clearBreadcrumbs } from './components/BreadcrumbLogger';

// Mock the stores since we can't run them in Node.js without React Native context
// In a real app, these would be the actual Zustand stores
...
interface AdminSettings {
  ...
  notifications: {
    email: { enabled: boolean; newJobs: boolean; jobUpdates: boolean; systemAlerts: boolean };
    push: { enabled: boolean; newJobs: boolean; jobUpdates: boolean; systemAlerts: boolean };
...
class MockAdminStore {
  ...
  updateSystemSettings(settings: Partial<AdminSettings['system']>) {
...
class MockBreadcrumbLogger {
  ...
  logBreadcrumb(route: string, userId?: string, metadata?: Record<string, any>) {
    const breadcrumb = {
      route,
...
// AdminBot Test Class
class AdminBot {
  private adminStore: MockAdminStore;
  private authStore: MockAuthStore;
  private breadcrumbLogger: MockBreadcrumbLogger;
  private testResults: Array<{ test: string; status: 'SUCCESS' | 'FAILURE'; details: any; errors?: any }> = [];
  ...
  private logResult(test: string, status: 'SUCCESS' | 'FAILURE', details: any, errors?: any) {
  ...
  async testAdminDualLoginToggle(): Promise<void> {
  ...
  async testBreadcrumbLogging(): Promise<void> {
  ...
  async testAdminStateManagement(): Promise<void> {
