...
const { generateSmartQuote, calculateLiveCost } = require('./utils/quote-generator');
const { getPartEstimate, getMotorcyclePartEstimate, calculatePartsTotal } = require('./utils/parts/getPartEstimate');
const { SERVICE_PRICING } = require('./constants/pricing');
...
class QuoteAIValidator {
  ...
  async testQuoteStatePersistence() {
    ...
    try {
      ...
      const states = ['pending', 'accepted', 'deposit_paid', 'paid'];
      ...
      this.log(`Error testing quote state persistence: ${error.message}`, false);
    }
  }
  ...
  async testPartsEstimation() {
    ...
    try {
      ...
      this.log(`Error calculating parts total: ${error.message}`, false);
    }
  }
...
module.exports = QuoteAIValidator;