...
// Import the license plate utilities
const fs = require('fs');
const path = require('path');

// Mock implementation since we can't import the actual TypeScript modules
...
// Mock implementation of decodePlateToVIN
async function decodePlateToVIN(plate, state) {
  ...
  // Check mock database
  const mockResult = mockPlateDatabase[normalizedPlate];
...
// Mock implementation of validatePlateFormat
function validatePlateFormat(plate, state) {
  const normalizedPlate = plate.toUpperCase().trim();
...
// Mock implementation of getSupportedStates
...
// Mock implementation of getStateName
...
const TEST_VINS = {
  ...
  cars: [
    { vin: '1HGBH41JXMN109186', expected: { make: 'Honda', vehicleType: 'car' } },
    { vin: '1FTFW1ET5DFC10312', expected: { make: 'Ford', vehicleType: 'car' } },
    { vin: 'WVWZZZ3BZWE689725', expected: { make: 'Volkswagen', vehicleType: 'car' } },
    { vin: '1G1ZT53806F109149', expected: { make: 'Chevrolet', vehicleType: 'car' } },
    { vin: 'JTDKN3DU8A0123456', expected: { make: 'Toyota', vehicleType: 'car' } },
  ],
  
  // Motorcycle VINs (17-character)
  motorcycles: [
    { vin: 'JH2RC5006LM200001', expected: { make: 'Honda', vehicleType: 'motorcycle' } },
    { vin: 'JYAVP31E8LA000001', expected: { make: 'Yamaha', vehicleType: 'motorcycle' } },
    { vin: 'JS1GR7JA5L2100001', expected: { make: 'Suzuki', vehicleType: 'motorcycle' } },
    { vin: 'JKA1KFD66A0000001', expected: { make: 'Kawasaki', vehicleType: 'motorcycle' } },
    { vin: '1HD1KEL19LB000001', expected: { make: 'Harley-Davidson', vehicleType: 'motorcycle' } },
  ],
  
  // Scooter VINs (Various lengths)
  scooters: [
    { vin: 'RFBLA4157L0000001', expected: { make: 'Vespa', vehicleType: 'scooter' } },
    { vin: 'L5YCGACB0A0000001', expected: { make: 'TaoTao', vehicleType: 'scooter' } },
    { vin: 'L6TCGACB0A0000001', expected: { make: 'Kymco', vehicleType: 'scooter' } },
    { vin: 'ZAPC31100', expected: { make: 'Piaggio', vehicleType: 'scooter' } }, // 9-character
    { vin: '5J6TC1200A', expected: { make: 'Yamaha', vehicleType: 'scooter' } }, // 10-character
  ],
...
function validateVin(vin, vehicleType) {
  if (vehicleType === 'car') {
...
function generateFallbackVinData(vin, vehicleType) {
  try {
...
function addTestResult(testName, passed, expected, actual, error, duration) {
  testResults.push({
    testName,
    passed,
    expected,
...
async function testVehicleTypeDetection() {
  ...
  for (const testCase of testCases) {
    const startTime = Date.now();
    const vinData = generateFallbackVinData(testCase.vin, testCase.expected);
    const duration = Date.now() - startTime;
...
async function testErrorHandling() {
  ...
  for (const testCase of errorTestCases) {
    ...
    try {
      const result = generateFallbackVinData(testCase.input, 'car');
      if (result === null) {
        errorCaught = true;
      }
    } catch (error) {
